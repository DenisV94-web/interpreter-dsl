...
```php
'get_static' => [
    'logging' => false,   // эндпоинт не пишет в логгер
    // ...
],
```

- `true` или ключ отсутствует — логирование включено (обратная совместимость);
- `false` — `logResult` и `logGlobalError` не пишут запись.
  Полезно для «горячих» эндпоинтов без бизнес-рисков (справочники, дашборды).
