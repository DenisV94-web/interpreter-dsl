# Блок execute

`execute` — бизнес-логика эндпоинта: ветвления, вызовы методов,
пропуски и формирование ответа. Выполняется после `mapping`.

```php
'execute' => [
    [ 'check' => 'if',     /* условие + действия */ ],
    [ 'check' => 'elseif', /* ... */ ],
    [ 'check' => 'else',   /* ... */ ],
],
```

---

## Цепочки ветвления (v1.13.0)

Блоки `execute` выполняются сверху вниз по правилам **цепочек**:

- `if` и `switch` **начинают новую независимую цепочку**;
- `elseif` и `else` **продолжают текущую цепочку**;
- внутри одной цепочки выполняется ровно один блок;
- несколько цепочек подряд выполняются по очереди;
- `else` привязывается к ближайшей открытой цепочке.

### Как это работает

```php
'execute' => [
    // Цепочка 1: классический if/elseif/else
    ['check' => 'if',     'filter' => [...], 'actions' => [...]],
    ['check' => 'elseif', 'filter' => [...], 'actions' => [...]],
    ['check' => 'else',                      'actions' => [...]],

    // Цепочка 2: независимый if (новый оператор)
    ['check' => 'if', 'filter' => [...], 'actions' => [...]],

    // Цепочка 3: switch (самостоятельный оператор)
    ['check' => 'switch', 'expression' => 'field:type', 'cases' => [...]],

    // Цепочка 4: ещё один if
    ['check' => 'if', 'filter' => [...], 'actions' => [...]],
],
```

Все цепочки выполняются **по очереди**. В каждой срабатывает
ровно один блок (или `default` у switch).

### Привязка else к ближайшей цепочке

```php
['check' => 'if', 'filter' => [/* истинно */], 'actions' => [A]],
['check' => 'switch', 'expression' => '...', 'cases' => [/* нет совпадений */]],
['check' => 'else', 'actions' => [B]],   // сработает: else относится к switch
```

`if` уже выполнился, `switch` не совпал — `else` срабатывает
(привязан к switch, а не к if).

### Сравнение со старой семантикой

| Старое поведение (до 1.13.0)                            | Новое поведение (1.13.0)             |
| ------------------------------------------------------- | ------------------------------------ |
| Два `if` подряд → второй пропускался                    | Оба выполняются по очереди           |
| `switch` после `if` → пропускался                       | Выполняется как новый оператор       |
| `if → switch(без совпадений) → else` → else пропускался | else срабатывает (привязан к switch) |

---

## Условие блока: два способа

### 1. `filter` — массив условий

```php
[
    'check'  => 'if',
    'filter' => ['field:client_string' => 'func:empty'],
    'actions' => [...],
]
```

Синтаксис — [Условия](../language/conditions.md)
(равенства, `!`, `func:*`, `AND/OR`, вложенность).

### 2. `method` — вызов метода

```php
[
    'check'  => 'elseif',
    'method' => 'isUnactive',
    'class'  => \Examples\DemoService::class,
    'params' => ['field:dealer_center_id'],
    'actions' => [...],
]
```

Результат вызова интерпретируется как булев. Удобно, когда
решение принимает код, а не сравнение полей.

---

## Действия (`actions`)

Блок содержит список действий; они выполняются по порядку.
Типы действий:

| Тип           | Как выглядит                  | Что делает                                                                 |
| ------------- | ----------------------------- | -------------------------------------------------------------------------- |
| Вызов метода  | `method` + `class` + `params` | Вызывает метод, пишет `lastResult`                                         |
| `set`         | `set`                         | Записывает значения в контекст                                             |
| `skip`        | `['skip' => true]`            | Запись `SKIPPED` в ответ, конец блока                                      |
| response-only | только `response`             | Пишет запись в ответ без вызова метода                                     |
| curl-действие | `curl`                        | HTTP-запрос (синтаксис как в [request.curl](request.md#curl-http-запросы)) |

### Вызов метода с response

```php
[
    'method'   => 'add',
    'class'    => \Api\Lead\Main::class,
    'params'   => ['mapping'],
    'response' => [
        'task_id'    => 'field:task_id',
        'lead_id'    => 'result',              // результат метода
        'date_create'=> 'field:datetime_now',
    ],
]
```

### set — запись в контекст (v1.12.0)

```php
[
    'set' => ['final_dep_id' => 'field:filtered_dep.ID|field:dep_new_check.ID'],
]
```

- выражения резолвятся как обычно: `field:`, цепочки `|`, `{{ }}`,
  литералы, массивы с `field:`;
- `conditions` на действии учитываются: условие ложно → запись
  не выполняется;
- значение доступно в следующих блоках и действиях как `field:имя`;
- `set` комбинируется с `method`/`response` в одном действии;
- действие только с `set` не считается пустым (без warning).

Кейс: цепочка запросов возвращает несколько кандидатов, а решение
«какой взять и применять ли» принимается в execute — результат
пишется в контекст и используется в update.

### skip

```php
'actions' => [['skip' => true]],
```

В ответ уходит `{"status": "SKIPPED", "iteration": N}`;
в итерационном режиме цикл переходит к следующей итерации.

### response-only (без метода)

```php
'actions' => [
    [
        'response' => [
            'task_id' => 'field:task_id',
            'error'   => 'Невалидный номер телефона',
        ],
    ],
],
```

Кейс: валидация не прошла — лид создавать нельзя, но в ответе
нужна запись с `task_id` и причиной. Метод не вызывается.

### Условия на отдельное действие

```php
[
    'conditions' => ['field:do_not_contact_flag' => 1],
    'method'     => 'update',
    'class'      => \Api\Contact\Main::class,
    'params'     => ['field:CONTACT.ID', [...]],
]
```

Ложное условие — действие пропускается, следующие выполняются.

---

## Выражения в `response`

| Выражение     | Значение                         |
| ------------- | -------------------------------- |
| `'result'`    | весь результат последнего метода |
| `'result:ID'` | поле результата                  |
| `'field:x'`   | поле контекста                   |
| литерал       | как есть                         |

Каждое действие с `response` добавляет **свою запись** в ответ —
response **дополняется**, а не перезаписывается, в том числе
из вложенных execute любой глубины
(в итерационном режиме — по записи на итерацию).

---

## merge_response — слияние записей response (v1.16.0)

По умолчанию каждое действие с `response` добавляет **отдельную запись**
в ответ:

```php
'execute' => [
    ['check' => 'if', 'actions' => [['response' => ['id_deal' => 'field:id_deal']]]],
    ['check' => 'if', 'actions' => [[
        'method'   => 'addCrmActivity',
        'class'    => \DesktopManager\Activity::class,
        'params'   => [...],
        'response' => ['activity_id' => 'result:ID'],
    ]]],
],
```

→ `[{"id_deal": 685026}, {"activity_id": 7469875}]` — две записи.

Опция на уровне экшена меняет семантику — все `response` объединяются
в одну запись:

```php
'QUALIFIED_NEW_CAR' => [
    'merge_response' => true,   // ← на уровне экшена, рядом с request/execute
    'request' => [...],
    'execute' => [...],
],
```

→ `[{"id_deal": 685026, "activity_id": 7469875}]` — одна запись.

### Поведение по режимам

| Режим        | Без `merge_response`                 | С `merge_response: true`                                                         |
| ------------ | ------------------------------------ | -------------------------------------------------------------------------------- |
| одиночный    | каждый `response` = отдельная запись | все `response` мержатся в одну запись                                            |
| итерационный | каждый `response` = отдельная запись | `response` внутри итерации мержатся в её запись; разные итерации = разные записи |

В итерационном режиме данные разных итераций **никогда не смешиваются**:
у каждой итерации своя запись, и только `response` внутри неё
объединяются.

Флаг пробрасывается во вложенные `execute` на любую глубину —
слияние работает одинаково на всех уровнях вложенности (v1.16.1).

### Когда включать

- экшен возвращает один логический объект, собранный из нескольких
  шагов (ID сделки + ID дела + статусы);
- клиент ждёт одну запись, а не список фрагментов.

### Когда НЕ включать

- экшен намеренно возвращает несколько независимых записей;
- итерационный режим, где каждая запись = результат отдельной итерации
  (поведение и так правильное без флага).

---

## Полный пример цепочки

```php
'execute' => [
    // 1. Пустые обязательные поля → пропуск
    [
        'check'   => 'if',
        'filter'  => ['logic' => 'OR',
                      'field:unified_client_id' => 'func:empty',
                      'field:segment_code'      => 'func:empty'],
        'actions' => [['skip' => true]],
    ],
    // 2. Неактивный ДЦ → пропуск (method-условие)
    [
        'check'   => 'elseif',
        'method'  => 'isUnactive',
        'class'   => \Examples\DemoService::class,
        'params'  => ['field:dealer_center_id'],
        'actions' => [['skip' => true]],
    ],
    // 3. Лид уже есть → обновить
    [
        'check'   => 'elseif',
        'filter'  => ['!field:LEAD.ID' => 'func:empty'],
        'actions' => [/* update + response */],
    ],
    // 4. Невалидный телефон → ошибка с task_id, без создания
    [
        'check'   => 'elseif',
        'filter'  => ['field:VALID_PHONE' => false],
        'actions' => [[
            'response' => ['task_id' => 'field:task_id',
                           'error'   => 'Невалидный номер телефона'],
        ]],
    ],
    // 5. Иначе → создать
    [
        'check'   => 'else',
        'actions' => [/* add + response */],
    ],
],
```

---

## Ошибки

- исключение в действии без `on_error` → ошибка итерации
  (в `partial` — запись `ERROR` в ответ, цикл продолжается)
  или глобальная ошибка в одиночном режиме;
- `on_error` у метода-действия работает так же, как в query.

---

## switch — ветвление по значению (v1.9.0)

Блок `switch` сравнивает разрешённое значение выражения с ключами
`cases` и выполняет actions совпавшего case (или `default`):

```php
'execute' => [
    [
        'check' => 'switch',
        'expression' => 'field:client_type',
        'cases' => [
            'contact' => [
                'actions' => [
                    ['method' => 'update', 'class' => \Api\Contact\Main::class, 'params' => [...]],
                ],
            ],
            'company' => [
                'actions' => [ /* ... */ ],
            ],
            'default' => [
                'actions' => [ /* ... */ ],
            ],
        ],
    ],
],
```

### Семантика

- `expression` резолвится через Field Resolver (`field:`, `{{ }}`, литералы);
- case проверяются **в порядке конфига**, сравнение слабое (`==`):
  строка `'1'` совпадает с числом `1`;
- **без fall-through**: выполнился один case — switch завершён;
- `default` выполняется, если ни один case не совпал;
- ни один case не совпал и нет `default` → switch «не сработал»:
  управление уходит следующему блоку (`elseif`/`else` этой цепочки
  или следующей цепочке);
- совпавший case прерывает цепочку — последующие `elseif`/`else`
  в этой же цепочке не выполняются.

---

## Вложенность execute (v1.10.0)

Action может содержать ключ `'execute' => [...]` — вложенный блок
ветвлений. Глубина не ограничена:

```php
'actions' => [
    ['response' => ['level' => 1]],
    [
        'execute' => [                          // уровень 2
            ['check' => 'switch', 'expression' => 'field:type', 'cases' => [
                'contact' => ['actions' => [
                    ['execute' => [             // уровень 3
                        ['check' => 'if', 'filter' => ['field:vip' => 1],
                         'actions' => [...]],
                        ['check' => 'else', 'actions' => [...]],
                    ]],
                ]],
            ]],
        ],
    ],
],
```

- вложенный execute работает везде, где разрешены actions:
  в if/elseif/else, в switch-case, в другом вложенном execute;
- `conditions` на action с `execute` учитываются;
- ошибка внутри вложенного блока прерывает весь `execute`.

---

## independent — legacy (v1.10.0)

Флаг `'independent' => true` сохранён для обратной совместимости:
блок проверяется всегда и не влияет на цепочки. С семантикой v1.13.0
для `if` и `switch` он больше не нужен — используй просто
последовательные блоки `if` или `switch`.

```php
'execute' => [
    ['check' => 'if', 'independent' => true,
     'filter' => ['field:need_log' => 1], 'actions' => [...]],
    ['check' => 'if', 'independent' => true,
     'filter' => ['field:need_notify' => 1], 'actions' => [...]],
],
```

- несколько независимых if подряд — срабатывают **все** истинные;
- независимый блок срабатывает, даже если цепочка уже выполнена;
- `'check' => 'switch'` с `independent` тоже работает;
- `'else'` с `independent` — не поддерживается (warning, блок пропущен).

---

## `'error'` — бизнес-ошибки в execute (v1.16.2)

Останавливает выполнение execute с пользовательским текстом.
Используется для бизнес-валидации, когда дальнейшая обработка
невозможна (нет воронки для конверсии, нет обязательного поля,
нарушено бизнес-правило).

```php
'execute' => [
    [
        'check' => 'if',
        'filter' => [
            'field:full_update.STATUS_ID' => 'CONVERTED',
            'field:category_id'           => 'func:empty'
        ],
        'actions' => [
            ['error' => 'У ответственного невозможно определить воронку!']
        ]
    ],
],
```

### Поведение

- текст резолвится: литерал, `field:x`, `{{x}}`;
- работает с `conditions` — если условия ложны, ошибка **не бросается**,
  и выполнение продолжается;
- все следующие блоки execute **пропускаются** (как при обычной ошибке);
- статус интерпретатора становится `ERROR`, текст попадает
  в `ResponseHandler->message`;
- работает во вложенных execute любой глубины.

### Отличие от `skip`

| Действие | Что делает                                             |
| -------- | ------------------------------------------------------ |
| `skip`   | пропускает итерацию (в итерационном режиме) без ошибки |
| `error`  | останавливает выполнение со статусом ERROR и текстом   |

### В связке с Runner

При вызове вложенного интерпретатора через `Runner` пользовательский
текст ошибки пробрасывается наружу: внешний интерпретатор получает
тот же текст в своём `ResponseHandler->message`.

---

## Куда дальше

- [compose](compose.md) — финальная сборка
- [Логирование](../components/logging.md) — как ошибки попадают в лог
